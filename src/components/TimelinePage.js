...
const ImageBox = ({ src, className = "", onClick }) => (
  <div 
...
const MediaBox = ({ src, className = "", onClick }) => {
  ...
  return <ImageBox src={src} className={className} onClick={onClick} />;
};
    ...
    <div className="site flex flex-col h-screen bg-black text-[#e6e6e6] relative overflow-hidden font-sans">
      ...
      {lightboxOpen && (
        ...
        >
          ...
          {lightboxImg.toLowerCase().endsWith('.mp4') || 
           lightboxImg.toLowerCase().endsWith('.webm') || 
           lightboxImg.toLowerCase().endsWith('.mov') ? (
            <video 
              src={lightboxImg} 
        ...
        {blocks.map((block, index) => {
          ...
          return (
            <div key={index} className="w-full flex flex-col gap-6">
              ...
              {(layout === 'TEXT_LEFT_IMAGE_RIGHT' || layout === 'TEXT_RIGHT_IMAGE_LEFT') && hasSubImages && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
                  {block.images.slice(1).map((img, i) => (
                    <MediaBox key={i} src={img} className="h-48" onClick={handleImageClick} />
                  ))}
                </div>
              )}
              ...
              {/* 6. GRID (Only images, grid layout) */}
              {layout === 'GRID' && (
                <div className="flex flex-col gap-6 w-full">
                  {(block.title || block.description) && (
                    <DescriptionBox title={block.title} description={block.description} className="w-full h-auto min-h-[150px]" />
                  )}
                  {block.images && block.images.length > 0 && (
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {block.images.map((img, i) => (
                        <MediaBox key={i} src={img} className="h-48 w-full object-cover" onClick={handleImageClick} />
                      ))}
                    </div>
                  )}
                </div>
